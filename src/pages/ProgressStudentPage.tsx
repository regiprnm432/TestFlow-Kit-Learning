...
import ProgressStudentTable from '../components/custom/ProgressStudentTable';
...
const ProgressStudentPage: React.FC = () => {
  ...
  const [students, setStudents] = useState<progress[]>([]);
  ...
  const handleSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setCurrentPage(1); 
    fetchDataProgress(1,e.target.value, selectedTopic)
  };

  const handlePageChange = (page: number) => {
    setCurrentPage(page);
    fetchDataProgress(page,searchQuery, selectedTopic)
  };

  const handleTopicChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    setSelectedTopic(e.target.value);
    setCurrentPage(1); // Reset to first page on topic change
    fetchDataProgress(1,searchQuery, e.target.value)
  };
  ...
  const fetchDataProgress = async (page:number, keyword:string, id_topik:string) => {
    setCurrentPage(page);
    setSearchQuery(keyword);
    try {
      const response = await fetch(`${apiUrl}/progress/search?keyword=${keyword}&id_topik=${id_topik}&page=${page}&limit=${itemsPerPage}`, {
        method: 'GET',
        headers: {
          'Accept': 'application/json',
          'Authorization': `Bearer ${apiKey}`
  ...
  const fetchDataTopics = async (id_topik:string) => {
    try {
      const response = await fetch(`${apiUrl}/progress/listTopik?id_topik=${id_topik}`, {
        method: "GET",
        headers: {
          Accept: "application/json",
          Authorization: `Bearer ${apiKey}`,
  ...
  useEffect(() => {
    if (session != null){
      if (session.login_type != "teacher"){
        ...
        fetchDataTopics(selectedTopic)
        fetchDataComboTopics()
        fetchDataProgress(1,searchQuery, selectedTopic)
      }
    }else{
      navigate("/login")
    }
...
export default ProgressStudentPage;
